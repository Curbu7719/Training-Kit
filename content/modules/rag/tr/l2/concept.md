...
- **Developer:** Yapı-farkındalıklı chunk'lamayı seçer, hybrid search ve re-ranking ekler, servis/branch metadata filtresi uygular ve ortada-kaybolmadan kaçınmak için bağlamı sıralar.
- **Enterprise Architect:** Belirli bir kullanıcının hangi kaynakları ve erişim seviyelerini retrieve edebileceğini ve genel retrieval mimarisini tasarlar.
- **Infrastructure Engineer:** Vektör index'ine/deposuna sahiptir, taze tutar ve bayat bir index'in nerede yanıltacağını işaretler.
- **Tester:** Retrieval'ı (doğru dosya çıktı mı?) generation'dan (sadık kullanıldı mı?) ayrı test eder ve her refactor'dan sonra bayat-index ve sınır-kaybı modlarını yoklar.
- **Project Manager:** Retrieval inceliğini (re-ranking, hybrid search) latency ve maliyete karşı tartar ve önce hangi depo ve wiki'lerin indexleneceğini önceliklendirir.
