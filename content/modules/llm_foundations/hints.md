...
- **H2:** Match the trait to its real-world consequence. If a trait is about *what the model can't be sure of* (a new framework version, factual truth), the implication is about *risk and verification*. If a trait is about *how output varies*, the implication is about *repeatability* — ask for a strict format and don't depend on an exact-string match.
- **H3:** For model-choice questions, map each SDLC requirement to a dial: "cheap at high volume" (commit messages) → smaller model; "identical every run" (structured output) → ask for a strict format and tolerate wording variation; "hard multi-step reasoning" (architecture review, gnarly refactor) → larger model. Pick the lightest option that satisfies the requirement.

## FAQ

**Q: Is a bigger model always the safer choice?**
A: No. A larger model costs more and is slower, and it still hallucinates. Use the smallest model that reliably does the task, and reserve the largest for genuinely hard work like design review or complex refactors.
...
A: LLMs are non-deterministic by default, so output can vary run to run. Don't rely on an exact-string match — design steps and tests that tolerate wording differences.
