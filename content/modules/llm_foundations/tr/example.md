...
**Görev 1 — commit mesajları üretmek.** Her push küçük bir prompt tetikler: *"Bu diff'i tek satırlık bir commit mesajı olarak özetle. Diff: ..."*. Model metni sürdürür ve en olası sonraki token'ları mesaj olur. Bu yüksek hacimli, basit ve iyi sınırlandırılmış bir iştir, bu yüzden ekip **küçük, hızlı, ucuz** bir modeli **düşük temperature** ile seçer — yaratıcılık değil, özlü ve tutarlı bir ifade isterler. Her commit'te amiral gemisi fiyatları ödemek savurganlık olurdu.

**Görev 2 — birim test senaryoları üretmek.** Bir test uzmanı, modele bir tarih ayrıştırma fonksiyonu için uç durumlar üretmesini söyler. Bu, bir commit mesajından daha fazla akıl yürütme gerektirir ama en zor görev değildir, bu yüzden **orta seviye** bir model uygundur. Temperature biraz daha yükseğe ayarlanır ki model bariz olanları tekrarlamak yerine *çeşitli* girdiler (artık yıllar, saat dilimleri, boş dizeler) önersin.
