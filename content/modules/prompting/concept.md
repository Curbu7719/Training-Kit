...
A **prompt** is the text you give a language model to steer its output. The model only ever sees the words you send — it cannot read your repository, your ticket tracker, or your intent. So the quality of those words largely decides the quality of the answer. Prompt engineering is the practice of writing prompts that are clear, structured, and testable, so the model behaves the way you want across the software lifecycle.
...
**Roles in a prompt.** Most systems separate a **system/developer message** from a **user message**. The system message sets lasting rules and persona — "You are a senior engineer who writes tests with the project's framework and never invents APIs." The user message carries the specific request — "Generate unit tests for this function." The system message has higher priority and stays in place across turns, so put policy, coding standards, and role there.

**Structure helps.** Models follow well-organized prompts more reliably:

- **Delimiters** — wrap the code, story, or spec in markers (triple backticks, XML-style tags) so the model can tell *your instructions* apart from *the thing to process*.
...
**Few-shot examples.** Showing one or more input→output examples ("few-shot") teaches the model your house style — your test naming, your story format — far better than describing it. Zero-shot (no examples) is fine for simple, clear tasks.

**Controlling output format.** Ask explicitly for a unified diff, a single test file, Gherkin scenarios, or JSON when downstream tooling or teammates use the result.

**Iterating.** A first prompt is a draft. Run it on varied inputs, find where it fails, and refine — like fixing code against failing tests, not casting magic spells.
