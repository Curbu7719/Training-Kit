...
- **Geliştirici/Mühendis:** Tool şemalarını (test paketini çalıştır, kod tabanında ara, bir yama uygula) tanımlar ve bir kodlama agent'ını güvende tutan çalıştırma katmanını artı döngü sınırlarını ve girdi doğrulamasını yazar.
- **İş Analisti:** Agent'ın ulaşması gereken hangi gerçek sistemleri ve veri kaynaklarını (görev takip sistemi, gereksinim deposu) ve iş akışında bir insan-onay adımının nereye ait olduğunu belirler.
- **PM/Ürün Sahibi:** Bir özelliğin gerçekten otonom bir agent'a mı yoksa daha ucuz bir tek çağrıya mı ihtiyaç duyduğuna karar verir, ne kadar otonomi alacağını kapsamlandırır ve iterasyon başına maliyeti bütçeler.
- **QA/Test Uzmanı & Mimar:** Kararsız (flaky) testleri triaj eden bir agent tanımlar, döngü sonlanmasını ve başarısızlık biçimlerini test eder ve yıkıcı veya kontrolsüz davranışı sınırlayan onay kapılarını ve izin verilen-tool sınırlarını belirler.
